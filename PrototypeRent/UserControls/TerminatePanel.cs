...
namespace PrototypeRent.UserControls
{
    public partial class TerminatePanel : UserControl
    {
        ...
        private void TerminatePanel_Load(object sender, EventArgs e)
        {
            LoadDataIntoDataGridView();
        }
        ...
        private void LoadDataIntoDataGridView()
        {
            ...
            try
            {
                ...
                string Query = "SELECT ID, Name, Contact_Number, Email, Building_Number, Room_Number, Status " + "FROM ContractsTable WHERE IsTerminated = 0 ORDER BY InsertedDate ASC";
        ...
        {
            string query = "UPDATE ContractsTable SET IsTerminated = 1, TerminationDate = @TerminationDate WHERE ID = @ID";
            string ConnectionString = "Data Source=SYLZAE;Initial Catalog=ContractDB;Integrated Security=True;Encrypt=True;TrustServerCertificate=True;";
            using (SqlConnection con = new SqlConnection(ConnectionString))
            {
                try
                {
                    ...
                    {
                        cmd.Parameters.AddWithValue("@TerminationDate", DateTime.Now);
